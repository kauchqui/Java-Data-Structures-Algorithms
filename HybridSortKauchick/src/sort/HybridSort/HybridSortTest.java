...
public class HybridSortTest
{
    ...
    void testAccuracy()
    {
        ...
        HybridSort<Integer> kauchick = new HybridSortChoi<>();

        Integer[][] input = {{0,1,2,3},{9,8,7,6},{0,3,1,2,4,5,6,7,8,9},{4,7,6,5},{9,8,11,10}}; //{0,1,2,3},{7,6,5,4},{0,3,1,2},{4,7,6,5},{9,8,11,10}
        testAccuracy(input, choi, kauchick);

        for (int row=10; row<=20; row++)
            for (int col=10; col<=20; col++)
                for (int i=0; i<100; i++)
                    testAccuracy(randomInput(row, col, 0.25), choi, kauchick);
    }
    ...
    void testSpeed()
    {
       ...
       HybridSort<Integer> choi = new HybridSortChoi<>();
        HybridSort<Integer> mine = new HybridSortKauchick<>();

        //for (col=100; col<=1500; col+=100)
        for (row=100; row<=1500; row+=100) //todo change back to 100/1,500
        {
